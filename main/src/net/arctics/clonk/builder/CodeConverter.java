...
public abstract class CodeConverter {
	...
	public static final class CodeConverterContext implements ICodeConverterContext, ITransformer {
		...
		private class FunctionAddition {
			...
			public String extraVariable(String baseName) {
				...
				final String mut = IntStream.iterate(0, x -> x + 1).mapToObj(x -> x == 0 ? baseName : baseName + x).filter(n -> (f == null || f.findVariable(n) == null) && !extraVariables.contains(n)).findFirst().get();
			...
			public ASTNode[] prelude(FunctionBody body) {
				final ASTNode varArray = this.varArray ? new VarDeclarationStatement(Arrays.asList(new VarInitialization(VAR_ARRAY_NAME, new ArrayExpression(), -1, -1, null, null)), Scope.VAR) : null;
				final ASTNode extraDecs = extraVariables != null && !extraVariables.isEmpty() ? new VarDeclarationStatement(extraVariables.stream().map(n -> new VarInitialization(n, null, -1, -1, null, null)).collect(Collectors.toList()), Scope.VAR) : null;
	...
	public void runOnDocument(final Script script, final IDocument document) {
