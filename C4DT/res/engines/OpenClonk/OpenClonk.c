static const int C4D_All;
static const int C4D_Background;
static const int C4D_Goal;
static const int C4D_Living;
static const int C4D_Object;
static const int C4D_Parallax;
static const int C4D_Rule;
static const int C4D_StaticBack;
static const int C4D_Structure;
static const int C4D_Vehicle;
static const int C4V_Array;
static const int C4V_Bool;
static const int C4V_C4Object;
static const int C4V_Int;
static const int C4V_Nil;
static const int C4V_PropList;
static const int C4V_String;
static const int COMD_Down;
static const int COMD_DownLeft;
static const int COMD_DownRight;
static const int COMD_Left;
static const int COMD_None;
static const int COMD_Right;
static const int COMD_Stop;
static const int COMD_Up;
static const int COMD_UpLeft;
static const int COMD_UpRight;
static const int DIR_Left;
static const int DIR_Right;
static const int NO_OWNER;
static const int OCF_Alive;
static const int OCF_AttractLightning;
static const int OCF_Available;
static const int OCF_Chop;
static const int OCF_Collectible;
static const int OCF_Collection;
static const int OCF_Construct;
static const int OCF_Container;
static const int OCF_CrewMember;
static const int OCF_Entrance;
static const int OCF_Exclusive;
static const int OCF_Fullcon;
static const int OCF_Grab;
static const int OCF_HitSpeed1;
static const int OCF_HitSpeed2;
static const int OCF_HitSpeed3;
static const int OCF_HitSpeed4;
static const int OCF_Inflammable;
static const int OCF_InFree;
static const int OCF_InLiquid;
static const int OCF_InSolid;
static const int OCF_Living;
static const int OCF_NotContained;
static const int OCF_OnFire;
static const int OCF_Rotate;
static const int this;
static const int C4V_Effect;
static const int C4V_Def;
static const int C4V_Function;
static const int C4X_Ver1;
static const int C4X_Ver2;
static const int C4X_Ver3;
static const int FX_OK;
static const int FX_Effect_Deny;
static const int FX_Effect_Annul;
static const int FX_Effect_AnnulDoCalls;
static const int FX_Execute_Kill;
static const int FX_Stop_Deny;
static const int FX_Start_Deny;
static const int FX_Call_Normal;
static const int FX_Call_Temp;
static const int FX_Call_TempAddForRemoval;
static const int FX_Call_RemoveClear;
static const int FX_Call_RemoveDeath;
static const int FX_Call_DmgScript;
static const int FX_Call_DmgBlast;
static const int FX_Call_DmgFire;
static const int FX_Call_Energy;
static const int FX_Call_EngScript;
static const int FX_Call_EngBlast;
static const int FX_Call_EngObjHit;
static const int FX_Call_EngFire;
static const int FX_Call_EngBaseRefresh;
static const int FX_Call_EngAsphyxiation;
static const int FX_Call_EngCorrosion;
static const int FX_Call_EngGetPunched;
static const int C4M_Vehicle;
static const int C4M_Solid;
static const int C4M_SemiSolid;
static const int C4M_Liquid;
static const int C4M_Background;
static const int SBRD_Caption;
static const int TEAM_Custom;
static const int TEAM_Active;
static const int TEAM_AllowHostilityChange;
static const int TEAM_Dist;
static const int TEAM_AllowTeamSwitch;
static const int TEAM_AutoGenerateTeams;
static const int TEAM_TeamColors;
static const int C4FO_Not;
static const int C4FO_And;
static const int C4FO_Or;
static const int C4FO_Exclude;
static const int C4FO_InRect;
static const int C4FO_AtPoint;
static const int C4FO_AtRect;
static const int C4FO_OnLine;
static const int C4FO_Distance;
static const int C4FO_ID;
static const int C4FO_OCF;
static const int C4FO_Category;
static const int C4FO_Action;
static const int C4FO_ActionTarget;
static const int C4FO_Procedure;
static const int C4FO_Container;
static const int C4FO_AnyContainer;
static const int C4FO_Owner;
static const int C4FO_Controller;
static const int C4FO_Func;
static const int C4FO_Layer;
static const int MD_DragSource;
static const int MD_DropTarget;
static const int MD_NoClick;
static const int C4SO_Reverse;
static const int C4SO_Multiple;
static const int C4SO_Distance;
static const int C4SO_Random;
static const int C4SO_Speed;
static const int C4SO_Mass;
static const int C4SO_Value;
static const int C4SO_Func;
static const int C4SECT_SaveLandscape;
static const int C4SECT_SaveObjects;
static const int C4SECT_KeepEffects;
static const int TEAMID_New;
static const int MSG_NoLinebreak;
static const int MSG_Bottom;
static const int MSG_Multiple;
static const int MSG_Top;
static const int MSG_Left;
static const int MSG_Right;
static const int MSG_HCenter;
static const int MSG_VCenter;
static const int MSG_DropSpeech;
static const int MSG_WidthRel;
static const int MSG_XRel;
static const int MSG_YRel;
static const int C4PT_User;
static const int C4PT_Script;
static const int CSPF_FixedAttributes;
static const int CSPF_NoScenarioInit;
static const int CSPF_NoEliminationCheck;
static const int CSPF_Invisible;
static const int DMQ_Sky;
static const int DMQ_Sub;
static const int DMQ_Bridge;
static const int PLRZOOM_Direct;
static const int PLRZOOM_NoIncrease;
static const int PLRZOOM_NoDecrease;
static const int PLRZOOM_LimitMin;
static const int PLRZOOM_LimitMax;
static const int C4D_Environment;
static const int C4D_MouseSelect;
static const int C4D_Foreground;
static const int C4D_MouseIgnore;
static const int C4D_IgnoreFoW;
static const int C4D_GrabGet;
static const int C4D_GrabPut;
static const int VIS_All;
static const int VIS_None;
static const int VIS_Owner;
static const int VIS_Allies;
static const int VIS_Enemies;
static const int VIS_Select;
static const int VIS_God;
static const int VIS_LayerToggle;
static const int VIS_OverlayOnly;
static const int C4MN_Style_Normal;
static const int C4MN_Style_Context;
static const int C4MN_Style_Info;
static const int C4MN_Style_Dialog;
static const int C4MN_Style_EqualItemHeight;
static const int C4MN_Extra_None;
static const int C4MN_Extra_Components;
static const int C4MN_Extra_Value;
static const int C4MN_Extra_Info;
static const int C4MN_Add_ImgRank;
static const int C4MN_Add_ImgIndexed;
static const int C4MN_Add_ImgObjRank;
static const int C4MN_Add_ImgObject;
static const int C4MN_Add_ImgTextSpec;
static const int C4MN_Add_ImgColor;
static const int C4MN_Add_PassValue;
static const int C4MN_Add_ForceCount;
static const int C4MN_Add_ForceNoDesc;
static const int GFXOV_MODE_None;
static const int GFXOV_MODE_Base;
static const int GFXOV_MODE_Action;
static const int GFXOV_MODE_Picture;
static const int GFXOV_MODE_IngamePicture;
static const int GFXOV_MODE_Object;
static const int GFXOV_MODE_ExtraGraphics;
static const int GFXOV_MODE_Rank;
static const int GFXOV_MODE_ObjectPicture;
static const int GFX_Overlay;
static const int GFXOV_Clothing;
static const int GFXOV_Tools;
static const int GFXOV_ProcessTarget;
static const int GFXOV_Misc;
static const int GFXOV_UI;
static const int GFX_BLIT_Additive;
static const int GFX_BLIT_Mod2;
static const int GFX_BLIT_ClrSfc_OwnClr;
static const int GFX_BLIT_ClrSfc_Mod2;
static const int GFX_BLIT_Wireframe;
static const int GFX_BLIT_Custom;
static const int GFX_BLIT_Parent;
static const int CNAT_None;
static const int CNAT_Left;
static const int CNAT_Right;
static const int CNAT_Top;
static const int CNAT_Bottom;
static const int CNAT_Center;
static const int CNAT_MultiAttach;
static const int CNAT_NoCollision;
static const int VTX_X;
static const int VTX_Y;
static const int VTX_CNAT;
static const int VTX_Friction;
static const int VTX_SetPermanent;
static const int VTX_SetPermanentUpd;
static const int C4OS_DELETED;
static const int C4OS_NORMAL;
static const int C4OS_INACTIVE;
static const int C4CMD_Base;
static const int C4CMD_SilentBase;
static const int C4CMD_Sub;
static const int C4CMD_SilentSub;
static const int C4CMD_MoveTo_NoPosAdjust;
static const int C4CMD_MoveTo_PushTarget;
static const int C4CMD_Enter_PushTarget;
static const int C4AVP_Const;
static const int C4AVP_Linear;
static const int C4AVP_X;
static const int C4AVP_Y;
static const int C4AVP_R;
static const int C4AVP_AbsX;
static const int C4AVP_AbsY;
static const int C4AVP_XDir;
static const int C4AVP_YDir;
static const int C4AVP_RDir;
static const int C4AVP_CosR;
static const int C4AVP_SinR;
static const int C4AVP_CosV;
static const int C4AVP_SinV;
static const int C4AVP_Action;
static const int ANIM_Loop;
static const int ANIM_Hold;
static const int ANIM_Remove;
static const int AM_None;
static const int AM_DrawBefore;
static const any nil;

global func any _inherited();
global func int Abs(int value);
global func int AbsX(int x);
global func int AbsY(int y);
global func bool ActIdle();
global func bool AddCommand(string command, object target, int x, int y, object target2, int delay, Data, int retries, int base_mode);
global func proplist AddEffect(string name, object target, int priority, int timer, object command_target, command_target_id, var1, var2, var3, var4);
global func int AddMenuItem(string caption, string command, id symbol, int count, parameter, string info_caption, int extra, XPar1, XPar2);
global func int AddMsgBoardCmd(string command, string script);
global func int AddVertex(int x, int y);
global func int Angle(int x1, int y1, int x2, int y2, int precision);
global func array Anim_AbsX(int position, int begin, int end, int length);
global func array Anim_AbsY(int position, int begin, int end, int length);
global func array Anim_Action();
global func array Anim_Const(int value);
global func array Anim_Linear(int position, int begin, int end, int length, int on_ending);
global func array Anim_R(int begin, int end);
global func array Anim_X(int position, int begin, int end, int length);
global func array Anim_XDir(int begin, int end, int max_dir);
global func array Anim_Y(int position, int begin, int end, int length);
global func array Anim_YDir(int begin, int end, int max_dir);
global func bool AppendCommand(string command, object target, any x, any y, object target2, int delay, Data, int retries, int base_mode);
global func int ArcCos(int value, int radius);
global func int ArcSin(int value, int radius);
global func int AttachMesh(mesh, string parent_bone, string child_bone, array transformation, int flags);
global func int BlastObjects(int x, int y, int level, object container, int caused_by);
global func int BoundBy(int value, int lower_bound, int upper_bound);
global func bool Bubble(int x, int y);
global func object Buy(id buy_object, int for_player, int pay_player, object to_base, bool show_errors);
global func id C4Id(string id_string);
global func any Call(function);
global func bool CanConcatPictureWith(object other);
global func int CastObjects(id, number, level, x, y, angle, angle_variance);
global func bool CastParticles(string name, int amount, int level, int x, int y, int a0, int a1, int b0, int b1, object obj);
global func int CastPXS(string material_name, int amount, int level, int x, int y, angle, angle_variance);
global func bool ChangeDef(id new_def);
global func bool CheckConstructionSite(id object_id, int xoffset, int yoffset);
global func int CheckEffect(string name, object target, int priority, int timer, var1, var2, var3, var4);
global func bool CheckVisibility(int player);
global func int ClearMenuItems();
global func bool ClearParticles(string name, object obj);
global func bool ClearScheduleCall(object obj, string function);
global func bool CloseMenu();
global func bool Collect(object item);
global func bool ComponentAll(id components);
global func object ComposeContents(id object_id);
global func any Construction(object creator);
global func object Contained();
global func object Contents(int index);
global func int ContentsCount(id object_id);
global func int Cos(int angle, int radius, int precision);
global func array CreateArray(int length);
global func object CreateConstruction(id object_id, int xoffset, int yoffset, int owner, int completion, bool adjust_terrain, bool check_site);
global func object CreateContents(id id, int count);
global func bool CreateMenu(id symbol, object command_object, int extra, string caption, int extra_data, int style, bool permanent, id menu_id);
global func object CreateObject(id id, int xoffset, int yoffset, int owner);
global func bool CreateParticle(string szName, int x, int y, int xdir, int ydir, int a, int b, object target, bool behind_target);
global func bool CreateParticleAtBone(string szName, string szBoneName, array pos, array dir, int a, int b, object target, bool behind_target);
global func bool CreateScriptPlayer(string name, int color, int team, int flags, id extra_data);
global func bool CustomMessage(string message, object obj, int plr, int offset_x, int offset_y, int color, id deco, portrait, int flags);
global func any DeathAnnounce();
global func any Definition(proplist def);
global func bool DetachMesh(int attach_number);
global func int DigFree(int x, int y, int radius, bool no_dig2objects);
global func int DigFreeRect(int x, int y, int width, int height, bool no_dig2objects);
global func int Distance(int x1, int y1, int x2, int y2);
global func bool DoBreath(int change);
global func any DoCon(int change);
global func any DoCrewExp(int change);
global func any DoDamage(int change, int damage_type, int caused_by);
global func any DoEnergy(int change, bool exact, int damage_type, int caused_by);
global func bool DoHomebaseMaterial(int player, id definition, int change);
global func bool DoHomebaseProduction(int player, id type, int change);
global func bool DoPlayerScore(int player, int change);

global func int DoRGBaValue(int Val, int change, int select);
global func bool DoScoreboardShow(int change, int for_player);
global func bool DoWealth(int player, int value);
global func bool DrawDefMap(int x, int y, int width, int height, string map_name);
global func bool DrawMap(int x, int y, int width, int height, string map);
global func bool DrawMaterialQuad(string szMaterialTexture, int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, bool sub);
global func int DrawParticleLine(string particle, int x0, int y0, int x1, int y1, int prtdist, int a, int b0, int b1, int ydir);
global func object EditCursor();
global func any EffectCall(object target, proplist effect, string command);
global func bool EliminatePlayer(int player);
global func bool Enter(object target);
global func any eval(string script);
global func bool Exit(int x, int y, int iR, int xdir, int ydir, int rdir);
global func any Explode(int level, id effect, string particle);
global func bool Extinguish();
global func int ExtractLiquid(int x, int y);
global func int ExtractMaterialAmount(int x, int y, int material_index, int amount);
global func bool FatalError(string message);
global func array Find_Action(string action);
global func array Find_ActionTarget(object action_target);
global func array Find_ActionTarget2(object action_target);
global func array Find_ActionTargets(object action_target1, object action_target2);
global func array Find_Allied(int player);
global func array Find_And(array Criteria);
global func array Find_AnyContainer();
global func array Find_AtPoint(int x, int y);
global func array Find_Category(int category);
global func array Find_Container(object container);
global func array Find_Distance(int radius, int x, int y);
global func array Find_Exclude(object exclude);
global func array Find_Func(string function);
global func array Find_Hostile(int player);
global func array Find_ID(id definition);
global func array Find_InRect(int x, int y, int width, int height);
global func array Find_NoContainer();
global func array Find_Not(array Criteria);
global func array Find_OCF(int ocf);
global func array Find_OnLine(int x1, int y1, int x2, int y2);
global func array Find_Or(array Criteria);
global func array Find_Owner(int player);
global func object FindBase(int player, int index);
global func array FindConstructionSite(id definition, int x, int y);
global func object FindContents(id type);
global func object FindObject(array Criteria);
global func array FindObjects(array Criteria);
global func object FindOtherContents(id type);
global func bool FinishCommand(bool success, int command_num);
global func any Fling(int xdir, int ydir);

global func string Format(string text);
global func int FrameCounter();
global func any FreeRect(int x, int y, int width, int height, int density);
global func bool GainMissionAccess(string password);
global func any GameCall(string function);
global func any GameCallEx(string function);
global func bool GameOver();
global func bool GBackLiquid(int x, int y);
global func bool GBackSemiSolid(int x, int y);
global func bool GBackSky(int x, int y);
global func bool GBackSolid(int x, int y);
global func string GetAction();
global func object GetActionTarget(int index);
global func any GetActMapVal(string entry, string action, id definition, int entry_number);
global func int GetActTime();
global func bool GetAlive();
global func int GetAnimationLength(string animation);
global func string GetAnimationName(int animation_number);
global func int GetAnimationPosition(int animation_number);
global func int GetAnimationWeight(int animation_number);
global func int GetBase();
global func bool GetBit(int old_value, int bit_number);
global func int GetBreath();
global func int GetCategory();
global func int GetChar(string text, int index);
global func int GetClimate();
global func int GetClrModulation(int overlay_id);
global func int GetColor();
global func int GetComDir();
global func any GetCommand(int element, int command_num);
global func int|id GetComponent(id component, int index, object obj, id definition);
global func int GetCon();
global func int GetContact(int index, int dwCNAT);
global func int GetController();
global func object GetCrew(int player, int index);
global func int GetCrewCount(int player);
global func bool GetCrewEnabled();
global func any GetCrewExtraData(string data_name);
global func object GetCursor(int player);
global func int GetDamage();
global func int GetDefBottom();
global func any GetDefCoreVal(string entry, string section, int entry_number);
global func id GetDefinition(int index);
global func int GetDir();
global func any GetEffect(string name, object target, int index, int max_priority);
global func int GetEffectCount(string mask, object target, int max_priority);
global func int GetEnergy();
global func bool GetEntrance();
global func int GetGravity();
global func object GetHiRank(int player);
global func int GetHomebaseMaterial(int player, id id, int index, int category);
global func int GetHomebaseProduction(int player, id id, int index, int category);
global func id GetID();
global func int GetIndexOf(array array2search, needle);
global func int GetKiller();
global func int GetLength(text);
global func int GetMass();
global func int GetMaterial(int x, int y);
global func int GetMaterialCount(int material_index);
global func any GetMaterialVal(string entry, string section, int material, int entry_number);
global func any GetMenu();
global func any GetMenuSelection();
global func string GetMeshMaterial(int submesh);
global func bool GetMissionAccess(string mission_access);
global func string GetName();
global func string GetNeededMatStr();
global func int GetObjectBlitMode(int overlay_id);
global func any GetObjectInfoCoreVal(string entry, string section, int entry_number);
global func any GetObjectVal(string entry, string section, int entry_number);
global func int GetOCF();
global func int GetOwner();
global func int GetPathLength(int startx, int starty, int endx, int endy);
global func int GetPhase();
global func int GetPhysical(string physical, int mode);
global func int GetPlayerByIndex(int index, int player_type);
global func int GetPlayerByName(string player_name);
global func string GetPlayerControlAssignment(int player, int control, bool human_readable, bool short_name);
global func int GetPlayerCount(int player_type);
global func int GetPlayerID(int player);
global func any GetPlayerInfoCoreVal(string entry, string section, int player, int entry_number);
global func string GetPlayerName(int player);
global func int GetPlayerScore(int player);
global func int GetPlayerScoreGain(int player);
global func int GetPlayerTeam(int player);

global func int GetPlayerType(int player);
global func any GetPlayerVal(string entry, string section, int player, int entry_number);
global func int GetPlrClonkSkin(int player);
global func int GetPlrColor(int player);
global func any GetPlrExtraData(int player, string data_name);
global func int GetPlrKnowledge(int player, id type, int index, int category);
global func any GetPlrMagic(int player, id id, int index);
global func object GetPlrView(int player);

global func int GetPlrViewMode(int player);
global func string GetProcedure();
global func array GetProperties(proplist object);
global func any GetProperty(string key, proplist object);
global func int GetR();
global func int GetRank();
global func int GetRDir(int precision);

global func int GetRGBaValue(int Val, int select);
global func int GetRootAnimation(int slot);
global func any GetScenarioVal(string entry, string section, int entry_number);
global func int GetSeason();
global func int GetSkyAdjust(bool back_color);
global func string GetTaggedPlayerName(int player);
global func int GetTeamByIndex(int index);
global func int GetTeamColor(int team);
global func int GetTeamCount();
global func string GetTeamName(int team);
global func int GetTemperature();
global func int GetTime();
global func int GetType(Value);
global func int GetUnusedOverlayID(int base_index);
global func int GetValue(object base, int for_player);
global func int GetVertex(int index, int data);
global func int GetVertexNum();
global func int GetWealth(int owner);
global func int GetWind(int x, int y, bool global);
global func int GetX();
global func int GetXDir(int precision);
global func int GetY();
global func int GetYDir(int precision);
global func bool GrabContents(object obj);
global func bool GrabObjectInfo(object from);
global func bool Hostile(int player1, int player2, bool check_one_way);
global func int HSL(int byH, int byS, int byL);
global func int HSL2RGB(int Val);
global func int HSLa(int byH, int byS, int byL, int byA);
global func bool Incinerate();
global func any inherited();
global func bool InLiquid();
global func bool InsertMaterial(int material_index, int x, int y, int xdir, int ydir, proplist out_insertpos);
global func bool Inside(int value, int lower_bound, int upper_bound);
global func bool IsNetwork();
global func bool Jump();
global func bool Kill(bool forced);
global func int LandscapeHeight();
global func int LandscapeWidth();
global func bool LaunchEarthquake(int x, int y);
global func bool LaunchLightning(int x, int y, int xspeed, int xrange, int yspeed, int yrange, bool do_gamma);
global func bool LaunchVolcano(int x, int y, int strength, string material_name);
global func bool Log(string message);
global func bool MakeCrewMember(int player);
global func int Material(string material_name);
global func string MaterialName(int material_index);
global func int Max(int value1, int value2);
global func bool Message(string message);
global func int Min(int value1, int value2);
global func int Music(string song_name, bool loop);
global func int MusicLevel(int level);
global func object Object(int object_number);
global func int ObjectCount(array Criteria);
global func int ObjectDistance(object obj2, object obj);
global func int ObjectNumber();
global func bool OnFire();
global func any Par(int index);
global func bool PathFree(int x1, int y1, int x2, int y2);
global func object PlaceAnimal(id animal_id);
global func int PlaceObjects(id definition, int amount, string material_name, int x, int y, int width, int height, bool on_surface, bool no_stuck);
global func object PlaceVegetation(id id, int x, int y, int width, int height, int growth);
global func int PlayAnimation(string animation, int slot, array position, array weight, int sibling);
global func bool PlayerMessage(int player, string message);
global func bool Punch(object obj, int strength);
global func bool PushParticles(string name, int xacc, int yacc);
global func int Random(int max);
global func int RandomX(int value1, int value2);
global func bool ReloadDef(id definition);
global func bool ReloadParticle(string particle);
global func int RemoveAll(array Criteria);
global func bool RemoveEffect(string name, object target, proplist effect, bool no_calls);
global func any RemoveObject(bool eject_contents);
global func int RemoveVertex(int index);
global func bool ResetGamma(int ramp_index);
global func bool ResetPhysical(string physical);
global func int RGB(int byR, int byG, int byB);
global func int RGB2HSL(int Val);
global func int RGBa(int byR, int byG, int byB, int byA);
global func bool Schedule(object obj, string script, int delay, int repeat);
global func bool ScheduleCall(object obj, string function, int delay, int repeat);
global func int ScoreboardCol(id definition);
global func object ScrollContents();
global func bool SelectMenuItem(int number);
global func any Sell(int player);
global func bool SetAction(string action, object target1, object target2, bool force);
global func bool SetActionData(data);
global func any SetActionTargets(object target1, object target2);
global func int SetAlive(bool alive);
global func bool SetAnimationPosition(int animation_number, array position);
global func bool SetAnimationWeight(int animation_number, array weight);
global func bool SetAttachBones(int attach_number, string parent_bone, string child_bone);
global func bool SetAttachTransform(int attach_number, array transformation);
global func int SetBit(int old_value, int bit_number, bool bit);
global func bool SetBridgeActionData(int bridge_length, bool move_clonk, bool build_wall, int bridge_material);
global func any SetCategory(int category);
global func any SetClimate(int temperature);
global func bool SetClrModulation(int color, int overlay_id);
global func any SetColor(int color);
global func int SetComDir(int com_dir);
global func bool SetCommand(string command, object target, int x, int y, object target2, Data, int retries);
global func bool SetComponent(id component, int count);
global func bool SetCon(int con);
global func bool SetContactDensity(int density);
global func bool SetController(int player);
global func any SetCrewEnabled(bool enabled);
global func any SetCrewExtraData(string data_name, Data);

global func bool SetCrewStatus(int player, bool into_crew);
global func bool SetCursor(int player, object obj, bool no_select_arrow);
global func int SetDir(int dir);
global func any SetEntrance(bool status);
global func int SetFilmView(int for_player);
global func any SetFoW(bool enabled, int player);
global func bool SetGameSpeed(int game_speed);
global func bool SetGamma(int color1, int color2, int color3, int ramp_index);
global func bool SetGraphics(string gfx_name, id source_definition, int overlay, int overlay_mode, string action, int dwBlitMode, object overlay_object);
global func any SetGravity(int gravity);
global func bool SetHostility(int player1, int player2, bool hostile, bool silent, bool no_calls);
global func bool SetKiller(int new_killer);
global func int SetLength(array resize, int size);
global func any SetMass(int mass);
global func bool SetMatAdjust(int colorMod);
global func bool SetMaxPlayer(int max);
global func bool SetMenuSize(int coloumns, int rows);
global func bool SetMeshMaterial(string material, int submesh);
global func bool SetName(string name, bool set_permanent, bool make_valid_if_exists);
global func bool SetObjDrawTransform(int width, int xskew, int xadjust, int yskew, int height, int yadjust, int overlay_id);
global func int SetObjectBlitMode(int dwNewBlitMod, int overlay_id);
global func bool SetOwner(int new_owner);
global func bool SetPhase(int phase);
global func bool SetPhysical(string physical, int value, int mode);
global func any SetPicture(int x, int y, int width, int height);
global func int SetPlayerTeam(int player, int new_team, bool no_calls);
global func bool SetPlayerViewLock(int player, bool locked);
global func bool SetPlayerZoomByViewRange(int player, int range_x, int range_y, int flags);
global func int SetPlayList(string playlist);
global func any SetPlrExtraData(int player, string data_name, Data);
global func int SetPlrKnowledge(int player, id definition, bool remove);
global func int SetPlrMagic(int player, id id, bool remove);
global func bool SetPlrView(int player, object obj);
global func any SetPlrViewRange(int range);
global func any SetPosition(int x, int y, bool check_bounds);
global func bool SetProperty(string key, value, proplist obj);
global func bool SetR(int angle);
global func bool SetRDir(int rdir, int precision);

global func int SetRGBaValue(int Val, int NewVal, int select);
global func bool SetScoreboardData(int row_id, int coloumn_id, string text, int data);
global func any SetSeason(int season);
global func any SetShape(int x, int y, int width, int height);
global func bool SetSkyAdjust(int colorMod, int backgroundColor);
global func bool SetSkyParallax(int mode, int xpar, int ypar, int xdir, int ydir, int x, int y);
global func any SetSolidMask(int x, int y, int width, int height, int target_x, int target_y);
global func bool SetSpeed(int xdir, int ydir, int precision);
global func any SetTemperature(int temperature);
global func bool SetTransferZone(int x, int y, int width, int height);
global func int SetVertex(int index, int select, int value, int set_custom_permanent);
global func bool SetVertexXY(int index, int xpos, int ypos);
global func bool SetViewOffset(int player, int x, int x);
global func bool SetWealth(int player, int value);
global func any SetWind(int strength);
global func bool SetXDir(int xdir, int precision);
global func bool SetYDir(int ydir, int precision);
global func any ShakeFree(int x, int y, int radius);
global func any ShakeObjects(int x, int y, int radius);
global func bool ShiftContents(bool shift_back, id target_id, bool no_calls);
global func bool ShowInfo();
global func array SimFlight(int x, int y, int xdir, int ydir, int min_density, int max_density, int iterations, int precision);
global func int Sin(int angle, int radius, int precision);
global func bool Smoke(int x, int y, int level);
global func array Sort_Distance(int x, int y);
global func array Sort_Func(string function);
global func array Sort_Mass();
global func array Sort_Multiple(array Criteria);
global func array Sort_Random();
global func array Sort_Reverse(array iSort);
global func array Sort_Speed();
global func array Sort_Value();
global func bool SortArray(array array2sort, bool descending);
global func bool SortArrayByArrayElement(array array2sort, int element_index, bool descending);
global func bool SortArrayByProperty(array array2sort, string property_name, bool descending);
global func bool SortScoreboard(int coloumn_id, bool reverse);
global func bool Sound(string name, bool global, int volume, int player, int loop_count);
global func bool Split2Components();
global func array SplitRGBaValue(int rgba);
global func int Sqrt(int value);
global func bool StartScriptProfiler(id definition_script);
global func bool StopAnimation(int animation_number);
global func bool StopScriptProfiler();
global func bool Stuck();
global func int Tan(int angle, int radius, int precision);
global func int ToggleBit(int old_value, int bit_number);
global func array Trans_Identity();
global func array Trans_Mul(array first, array second);
global func array Trans_Rotate(int angle, int rx, int ry, int rz);
global func array Trans_Scale(int sx, int sy, int sz);
global func array Trans_Translate(int dx, int dy, int dz);
global func string Translate(string text);
global func int VerticesStuck(object obj);
global func string WildcardMatch(string text, string wildcard);
global func any CreatePropList(prototype);
global func any ResetProperty(key, any pObj);
global func any DebugLog(Pars);
global func any AsyncRandom(int iRange);
global func any LocateFunc(funcname, any p);
global func any ModulateColor(int iClr1, int iClr2);
global func any StartCallTrace();
global func any GetConstantNameByValue(int value, name_prefix, int idx);
global func any IncinerateLandscape(int iX, int iY);
global func any GetPlayerColor(int iPlayer);
global func any GetTexture(int x, int y);
global func any GetAverageTextureColor(string Texture);
global func any BlastFree(int iX, int iY, int iLevel, int iCausedBy);
global func any AddMessage(Pars);
global func any ResetCursorView(int plr);
global func any SurrenderPlayer(int iPlr);
global func any SetLeaguePerformance(int iScore);
global func any GetViewCursor(int iPlr);
global func any SetViewCursor(int iPlr, any pObj);
global func any ClearFreeRect(int iX, int iY, int iWdt, int iHgt);
global func any PathFree2(x1, y1, x2, any y2);
global func any GetLeague(int idx);
global func any TestMessageBoard(int iForPlr, bool fTestIfInUse);
global func any CallMessageBoard(pObj, bool fUpperCase, szQueryString, int iForPlr);
global func any AbortMessageBoard(pObj, int iForPlr);
global func any OnMessageBoardAnswer(pObj, int iForPlr, any szAnswerString);
global func any GetMatAdjust();
global func any DrawMatChunks(int tx, int ty, int twdt, int thgt, int icntx, int icnty, strMaterial, strTexture, bool bIFT);
global func any CastAParticles(szName, int iAmount, int iLevel, int iX, int iY, int a0, int a1, int b0, int b1, pObj, bool fBack);
global func any CastBackParticles(szName, int iAmount, int iLevel, int iX, int iY, int a0, int a1, int b0, int b1, any pObj);
global func any SetTextureIndex(psMatTex, int iNewIndex, bool fInsert);
global func any RemoveUnusedTexMapEntries();
global func any LoadScenarioSection(pstrSection, int dwFlags);
global func any SetPreSend(int iToVal, any pNewName);
global func any GetTeamConfig(int iConfigValue);
global func any InitScenarioPlayer(int iPlayer, int idTeam);
global func any GetScoreboardString(int iRowID, int iColID);
global func any GetScoreboardData(int iRowID, int iColID);
global func any AddEvaluationData(pText, int idPlayer);
global func any HideSettlementScoreInEvaluation(bool fHide);
global func any ActivateGameGoalMenu(int iPlayer);
global func any PauseGame(bool fToggle);
global func any SetNextMission(szNextMission, szNextMissionText, any szNextMissionDesc);
global func any GetPlayerControlState(int iPlr, int iControl);
global func any SetPlayerControlEnabled(int iplr, int ctrl, bool is_enabled);
global func any GetPlayerControlEnabled(int iplr, int ctrl);
global func any ExecuteCommand();
global func any SetMenuDecoration(idNewDeco);
global func any SetMenuTextProgress(int iNewProgress);
global func any GetObjectLayer();
global func any SetObjectLayer(pNewLayer);
global func any SetObjDrawTransform2(int iA, int iB, int iC, int iD, int iE, int iF, int iG, int iH, int iI, int iOverlayID);
global func any SetObjectStatus(int iNewStatus, bool fClearPointers);
global func any GetObjectStatus();
global func any AdjustWalkRotation(int iRangeX, int iRangeY, int iSpeed);
global func any DoNoCollectDelay(int change);
global func any this();

static const proplist MapLayer;
static const proplist Map;
static const int MAPALGO_Layer;
static const int MAPALGO_RndChecker;
static const int MAPALGO_And;
static const int MAPALGO_Or;
static const int MAPALGO_Xor;
static const int MAPALGO_Not;
static const int MAPALGO_Scale;
static const int MAPALGO_Offset;
static const int MAPALGO_Rect;
static const int MAPALGO_Ellipsis;
static const int MAPALGO_Polygon;
static const int MAPALGO_Turbulence;
static const int MAPALGO_Border;

static const int GUI_SetTag;
static const int GUI_Call;
static const int GUI_GridLayout;
static const int GUI_VerticalLayout;
static const int GUI_TextVCenter;
static const int GUI_TextHCenter;
static const int GUI_TextRight;
static const int GUI_TextBottom;
static const int GUI_TextTop;
static const int GUI_TextLeft;
static const int GUI_FitChildren;
static const int GUI_Multiple;
static const int GUI_IgnoreMouse;
static const int GUI_NoCrop;

global func int CustomGuiOpen(proplist menu);
global func bool CustomGuiSetTag(string tag, int menuID, int childID, object target);
global func bool CustomGuiClose(int menuID, int childID, object target);
global func bool CustomGuiUpdate(proplist update, int menuID, int childID, object target);

global func bool Draw(string mattex, proplist mask_algo, array rect);
global func bool Blit(proplist mask_algo, array rect);
global func proplist CreateLayer(string mattex_fill, int width, int height);
global func proplist Duplicate(mask_spec, array rect);
global func int GetPixel(int x, int y);
global func bool SetPixel(int x, int y, to_value);
global func int GetPixelCount(mask_spec, array rect);
global func bool Resize(int new_wdt, int new_hgt);
global func bool FindPosition(proplist out_pos, mask_spec);
global func array CreateMatTexMask(mask_spec);

static const object Global;